...
├── war-academy.html           # 戦争学園計算（研究: 盾兵10項目 ／ 建物: 火晶強化。火晶計算ツールと同じ式。データは公式Wiki）
...
`JL_BASE` holds the 17 whole-level entries (index 0 = level 0, 1–16 = gem levels), same as the original data. `JL` is generated from it at load time: index 0–4 map 1:1 to levels 0–4 unchanged, but from level 4 onward each level-up is split into `GEM_SUB` (4) sub-stages (4.0→4.1→4.2→4.3→5.0→…→16.0 — reaching all 4 dots filled at level N *is* level N+1 at 0 dots, not a separate state), so `JL` ends up with 53 entries and `JL_MAX` is 52. Material cost per whole-level entry in `JL_BASE` is divided evenly across its 4 sub-stage entries (`splitEven`); `lethality`/`hp` only change at the whole-level boundary (the 4th sub-stage), not on every sub-stage, since the buff itself doesn't increase until the level is fully reached. `jlLabel(idx)` converts a `JL` index back to `{level, sub}` for display, and `gemDots(sub)`/`gemDotsHtml(sub)` render the sub-stage progress as `●○` dots (2 dots × 2 lines, to fit narrow boxes without clipping). `jlIndexFromLabel(level, sub)` is the inverse, used by the bulk-input modal's per-slot level-jump (⏫/⏬, ±1 whole level keeping the same sub-stage) alongside the existing single-step (▲/▼, ±1 raw index) buttons.
